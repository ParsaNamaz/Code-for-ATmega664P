...
int main(void)
{
	// read input data from Port B
	// write bits 7:4
	DDRB = 0x00;

	// digits a-g, "." are connected to PORTA
	DDRA = 0xFF;
	//Basis for the Transistors
	...
	while (1)
	{
		uint8_t digits_in = PINB ;
		
		
		
		uint8_t dot_a_g = BCD_TABLE[digits_in];
		
		if (count >= 1) // count for 1 second (assuming prescaler is set to 1024)
		{
			count = 0; // reset count value
			static uint8_t num = 0; // static variable to remember last displayed digit

			num++; // increment number
			 // reset number if greater than 3
			if(num>2){
				num=0;
			}
			if(num==0){
			PORTC =0x04;
			PORTA = BCD_TABLE[(digits_in/100)]; // display number on 7-segment display
			}
			if(num==1){
			PORTC=0x02;
			PORTA = BCD_TABLE[(digits_in/10)%10];
			}
			if(num==2){
			PORTC=0x01;
			PORTA = BCD_TABLE[digits_in%10];
			}
		}
	}
}
